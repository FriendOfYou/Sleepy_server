...
from app import app
from app.mysql_data import search_personList
...
def list_person():
    ...
    person_data = search_personList()
    if person_data != 0:
        persons = []
        if page * size < len(person_data):
            ...
            for i in range(size * (page - 1), len(person_data)):
                person = {'id': person_data[i][0], 'name': person_data[i][1], 'sex': person_data[i][3],
                          'birthday': person_data[i][4], 'birthplace': person_data[i][5], 'summary': person_data[i][6],
                          'img': person_data[i][2]}
                persons.append(person)
        total = int(len(person_data) / size)
        if total * size != len(person_data):
            total = total + 1
        return Response(
