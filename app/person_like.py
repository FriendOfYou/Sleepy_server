...
from app import app
from app.mysql_data import judge_Personlike, update_Personlike, set_Personlike
...
def person_like(person_id):
    ...
    if request.method == 'GET':
        ...
        if like != 0:
            if like[2] == 1:
                return Response(json.dumps({'status': 0, 'msg': "该用户喜欢该影人", 'like': 1}),
                                content_type='application/json')
            elif like[2] == -1:
                return Response(json.dumps({'status': 0, 'msg': "该用户不喜欢该影人", 'like': -1}),
                                content_type='application/json')
            else:
                return Response(json.dumps({'status': 0, 'msg': "该用户未对该影人评价", 'like': 0}),
                                content_type='application/json')
        else:
            set_Personlike(uid, person_id)
            return Response(json.dumps({'status': 0, 'msg': "该用户未对该影人评价", 'like': 0}),
                            content_type='application/json')
    # 设置电影标记喜欢/不喜欢
    else:
        ...
        if update_Personlike(person_id, uid, like_choice) == 1:
            return Response(json.dumps({'status': 0, 'msg': "影人标记修改成功"}),
                            content_type='application/json')
        else:
