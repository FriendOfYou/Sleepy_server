...
#
#
# def get_Countryid(country_name):
#     conn = mysql_conn()
#     cursor = conn.cursor()
#     sql = "select * from country where country_name='%s'" % country_name
#     try:
#         cursor.execute(sql)
#         if cursor is not None:
#             row = cursor.fetchone()
...
#
#
# def insert_genre(movie_id, country_id, country_name):
#     conn = mysql_conn()
#     cursor = conn.cursor()
#     sql = "insert into movie_countries(movie_id,country_id,country_name) values(%d,%d,'%s')" % (movie_id, country_id, country_name)
#     try:
#         cursor.execute(sql)
#         conn.commit()
#     except Exception as e:
...
#     k = a[i][9]  # country单元   [8]genre
#     k = re.split('/', k)
#     for j in range(len(k)):
#         id_g = get_Countryid(k[j])
...
# #     print(id_g)
a="movie_genres.genre_name='%s' OR "
a=a[:-3]
print(a)
